  ...
  if (rails.fire($(document), 'rails:attachBindings')) {
    ...
    ! function (a, b, c, d) {
        ...
        e.Defaults = { items: 3, loop: !1, center: !1, rewind: !1, mouseDrag: !0, touchDrag: !0, pullDrag: !0, freeDrag: !1, margin: 0, stagePadding: 0, merge: !1, mergeFit: !0, autoWidth: !1, startPosition: 0, rtl: !1, smartSpeed: 250, fluidSpeed: !1, dragEndSpeed: !1, responsive: {}, responsiveRefreshRate: 200, responsiveBaseElement: b, fallbackEasing: "swing", info: !1, nestedItemSelector: !1, itemElement: "div", stageElement: "div", refreshClass: "owl-refresh", loadedClass: "owl-loaded", loadingClass: "owl-loading", rtlClass: "owl-rtl", responsiveClass: "owl-responsive", dragClass: "owl-drag", itemClass: "owl-item", stageClass: "owl-stage", stageOuterClass: "owl-stage-outer", grabClass: "owl-grab" }, e.Width = { Default: "default", Inner: "inner", Outer: "outer" }, e.Type = { Event: "event", State: "state" }, e.Plugins = {}, e.Workers = [{ filter: ["width", "settings"], run: function () { this._width = this.$element.width() } }, { filter: ["width", "items", "settings"], run: function (a) { a.current = this._items && this._items[this.relative(this._current)] } }, { filter: ["items", "settings"], run: function () { this.$stage.children(".cloned").remove() } }, {
            ...
            var d;
            return this.options.responsiveBaseElement !== b ? d = a(this.options.responsiveBaseElement).width() : a(b).width() ? d = a(b).width() : c.documentElement && c.documentElement.clientWidth ? d = c.documentElement.clientWidth : console.warn("Can not detect viewport width."), d
        }, e.prototype.replace = function (b) {
            this.$stage.empty(), this._items = [], b && (b = b instanceof jQuery ? b : a(b)), this.settings.nestedItemSelector && (b = b.find("." + this.settings.nestedItemSelector)), b.filter(function () {
                return 1 === this.nodeType
            }).each(a.proxy(function (a, b) { b = this.prepare(b), this.$stage.append(b), this._items.push(b), this._mergers.push(1 * b.find("[data-merge]").addBack("[data-merge]").attr("data-merge") || 1) }, this)), this.reset(this.isNumeric(this.settings.startPosition) ? this.settings.startPosition : 0), this.invalidate("items")
...
function adaptive() {
    var header_top = $('.header_top'),
        ...
        mainnewscolumn = $('#mainnewscolumn');

    var current_width = getCurrentWidth();

    var online_url = 'http://ice912.echo.msk.ru:9120/stream';

    header_top.after('<a class="version_switcher" id="go_to_full">Перейти на полную версию сайта</a>');

    // burger menu
    header_top.find('.wrapper').append('<a class="hamburger"><span class="line" /><span class="line" /><span class="line" /></a>');
    header_top.find('.mmsearch').insertBefore(header_top.find('.mainmenu'));
    header_top.on('touchstart', '.hamburger', function () {
        $(this).toggleClass("m-active");
        header_top.find('.mmsearch, .mainmenu').toggle();
    });

    $('.mainmenu .menubar').prepend( $('.secondmenu .menubar').html() );

    // Sidebar
    if (mainnewscolumn.length) {
        mainnewscolumn.after('<div class="sidebar"><div id="adv_0"></div></div>');
    } else {
        $('.main').before('<div class="sidebar"><div id="adv_0"></div></div>');
    }

    adv_insert('adv_0');

    mainsidebar.insertAfter('#onair').show();

    // Поток
    // $('a.playerbutton').removeClass('playerbutton').attr('href', online_url);

    // Слайдер Главные новости
    $('.supermaincarousel').addClass('owl-carousel owl-theme').owlCarousel({
        margin: 0,
        touchDrag: true,
        autoHeight: true,
        responsive: {
    ...
    // Слайдер Важные люди
    // $('.vip_showcase').html($('.vip_showcase').find('.mainperson')).addClass('owl-carousel owl-theme').owlCarousel({
    //     margin: 0,
    //     touchDrag: true,
    //     items: 1,
    //     autoHeight: true
    ...
    $('#onair').on('click', 'h2', function (e) {
        location.href = '/schedule/'
    });

    $("iframe[src*='videos']").on('load', function () {
        var iframe = $(this);

        iframe.css({ 'width': current_width - 40, 'height': current_width - 40 });

        setTimeout(function () {
            var iframe_content = iframe.contents();

            iframe_content
                .find('[id*=otv_play], .content_video-dimensions')
                .css({ 'width': current_width - 40, 'height': current_width - 40 });

            iframe_content
                .find('video')
                .css({ 'width': current_width - 40, 'top': 0 });

            iframe_content
                .find('body')
                .css({ 'margin': 0 });

            (current_width < 360) && iframe_content
                .find('.vjs-remaining-time')
                .addClass('vjs-hidden');
        }, 5000)
    });

    // Расстановка баннерных мест на главной странице
    if (window.location.pathname === '/') {
        ...
        if (current_width <= 480) {
            pos = '2n';
        } else if (current_width <= 720) {
            pos = '4n';
        } else {
            pos = '6n';
        }

        $('.mainpreview').find('.preview:nth-child(' + pos + ')').after(function () {
            if (i > 10) {
                return;
            }
            return '<div id="adv_' + (i++) + '" class="adv_banner"></div>';
        });
...
// Конфиг для соц. кнопок
var addthis_config = {
    services_compact: 'email,link,telegram,google_plusone_share,more'
}

$(function () {
    // В Опера-мини показываем полную версию всегда
    if (window.isOperaMini) {
        return
    }

    if (getCurrentWidth() <= getMaxWidth()) {
        if (window.is_adaptive) {
            adaptive();
        } else {
            $('footer .copy').before('<a class="version_switcher" id="go_to_adaptive">Перейти на мобильную версию сайта</a>')

            if (!getCookie('not_adaptive_header')) {
                $('.header').before('<div class="version_switcher_header">Показать мобильную версию? <a data-res="1" href="#">Да</a> <a data-res="0" href="#">Нет</a> </div>');
            }
        }
    }

    $('body').on('click', '.version_switcher_header a', function (e) {
        e.preventDefault();

        // Прячем кнопку на день
        setCookie('not_adaptive_header', '1', { expires: 60 * 60 * 24 });

        if ($(this).data('res') === 0) {
            $('.version_switcher_header').remove();
        } else {
            $('#go_to_adaptive').trigger('click');
        }
    });

    // Кнопка перехода на полную версию и обратно
    $('body').on('click', '#go_to_adaptive', function (e) {
        e.preventDefault();
        
        setCookie('not_adaptive', '0');

        if (window.ga && ga instanceof Function) {
            ga('send', 'event', 'switch', 'to_responsive');
        }

        location.reload();
    }).on('click', '#go_to_full', function (e) {
        e.preventDefault();
        
        setCookie('not_adaptive', '1');

        setCookie('not_adaptive_header', '1', { expires: 60 * 60 });

        if (window.ga && ga instanceof Function) {
            ga('send', 'event', 'switch', 'to_desktop');
        }
...
/**
 * Если баннер больше 50%, убираем float
 */
$(function() {
    $('.adv_inside, .video_adv_1').on('DOMSubtreeModified', function() {
        var wrapper = $(this),
            children = wrapper.children('div'),
            float = 'left';
...
$.fn.settingsform = function(options){
	this.each(function(){
		var form = $(this);
		// попапы для смены email и password
			...
			if (cont.hasClass('changeemail')) {
				...
				cont.find('.button').on('click', function(e){
					...
					if (val.length<1) {
						...
						cont.find('.error').css('display','none');
// заглушка для сохранения емайла 
						$.ajax({
							url: cont.data('path'),
							data: $(':input',cont).serialize(),
							method: 'POST',
...
_MOBILEPOPUP = {'link':'http://m.echo.msk.ru/',
		'text':'Вы можете перейти<br />на мобильную версию сайта', // HTML is allowed
		'yes':'Перейти',
		'no':'Отказаться',
...
$(document).ready(function() {
	...
	e.preventDefault() ;
      ...
      $(document).on('change', '.c_select', function (e) {
	...
	var self = $(this) ;
	if (state == 1) {
	  self.html('<span class="wsico">&nbsp;</span>не рекомендовать') ;
	} else {
	  self.html('<span class="wsico">&nbsp;</span>рекомендовать') ;
	} ;
      }) ;
	...
	function (e) {
	  ...
	  if (self.hasClass('on-state')) {
	    ...
	    if (self.hasClass('flag') &&
	      !confirm('Действительно пожаловаться?')) {
       	      e.preventDefault() ;
	      return ;
	    }
	    if (self.hasClass('ignor')) {
	      if (confirm('Вы уверены, что хотите внести пользователя в чёрный список?')) {
	        self.hide() ;
	      } else {
	        e.preventDefault() ;
		return ;
	...
	var self = $(this) ;
	if (confirm('Предложить забанить этого пользователя?')) {
	...
	$.each(udata.foes, function (i, v) {
	  ...
	  $('[data-author="' + v + '"]').each(function (i, cmnt) {
	    ...
	    if (self.hasClass('bcast-q')) {
	      self.html('(вопрос проигнорирован)') ;
	    } else {
	      self.html('(комментарий проигнорирован)') ;
	    }
	  }) ;
	}) ;
      } ;
	...
	if ((udata.own_id != author) && udata.liked && udata.flagged) {
	  if (udata.liked.indexOf(btn_bar.attr('id')) >= 0) {
	    like_btn.addClass('on-state').attr('title', 'Вы поддержали') ;
	  } ;
	  if (mlevel == 3) {
	  ...
	  if (access_level >= 2) { // can ignore
	    self.find('.author a.dark').after(
	      '<a href="/foes/' + author + '" class="ignor">Игнорировать</a>') ;
	  } ;
	  if (access_level >= 3 && access_level < 6) { //can flag
	    btn_bar.append('<a ref="nofollow" href="/flags/' +
	      btn_bar.attr('id') + '" class="red flag">Пожаловаться</a>') ;
	...
	if (access_level > 3 && access_level > mlevel) {
	  if (hidden) {
	    btn_bar.append(
	      ...
	      btn_bar.attr('id') +
	      '/unhide.html" class="red hide">(Показать)</a>') ;
	  } else {
	    btn_bar.append(
	      '<a rel="nofollow" href="/moderate/comments/' +
	      btn_bar.attr('id') +
	      '/hide.html" class="red hide">(Скрыть)</a>') ;
	  } ;
	  if (access_level < 6) {
	    if (autor_div.size() > 0 && udata.own_id != author) {
	      btn_bar.append('<a rel="nofollow" href="/moderate/comments/' +
		  btn_bar.attr('id') +
		  '/ban_request.html" class="red ban-request">(Предложить бан)</a>') ;
	    } ;
	  } else {
	    btn_bar.append(
	      '<a rel="nofollow" href="/moderate/comments/' +
	      btn_bar.attr('id') +
	      '.html" class="red hide delete">(Удалить)</a>') ;
	    if (autor_div.size() > 0) {
	      btn_bar.append(
		'<a rel="nofollow" href="/moderate/comments/' +
		btn_bar.attr('id') +
		'/ban.html" class="red hide">(Забанить)</a>') ;
	    } ;
	  } ;
	} ;
	if (access_level > 3 && autor_div.size() == 0) {
	  btn_bar.append('<strong>(Забанен)</strong>') ;
	} ;
      }) ;
    ...
    (function () {
        ...
        $('.fileupload input[type=file]').on('change', function (e) {
            ...
            $.ajax({
                ...
                contentType: false,
                processData: false,
                error: function () { alert('Ошибка загрузки файла') ; },
                success: function (html) {
                    $('.pic', sav).html(html) ;
                    $('a.delete', sav).show() ;
                }
    ...
    $('.filtrsearch input').click(function () {
      var searchCond = $('#search_cond_search_' + $(this).val()) ;
	...
	if ($(this).find('span.checked').size() == 0) {
	  alert('Пожалуйста, выберите ваш вариант ответа') ;
	  return false ;
	} ;
        return true ;
    }) ;
    ...
    if (window.localStorage)
      (function () {
        firebase.initializeApp({
          messagingSenderId: '915935606312'
        }) ;
        var fcm = firebase.messaging() ;

        navigator.serviceWorker.register('/sw.js').
          then(function (reg) { fcm.useServiceWorker(reg); });

        fcm.usePublicVapidKey('BKRseGi3yZbqLVV1Zqo3RnZI_HxX_S3gKwxbZY64_brTxzgzhvcLkyzMODTSKDZQfvWPGsMlTN8Rawpu21d1i8k') ;
        // console.log(fcm) ;

        var oldPerm = window.localStorage.getItem('msg_ax') || 'default' ;

        function setPermState(newPerm) {
          if (newPerm == 'default') {
            window.localStorage.setItem('msg_ts',
              (new Date()).getTime().toString()) ;
          } else {
            window.localStorage.removeItem('msg_ts') ;
          }

          if (newPerm == oldPerm) return ;
          console.log('setting msg_ax to', newPerm) ;
          window.localStorage.setItem('msg_ax', newPerm) ;
          setTimeout(function () {
            $(document).trigger('push:subscription', newPerm) ;
          }, 1000) ;
        }

        function getToken(ask) {
          fcm.getToken().then(function (token) {
            if (!token) {
              console.log('no fcm token') ;
              if (ask) getPermission()
              return ;
            }
            if (window.localStorage.getItem('fcm_id') == token) {
              console.log('known fcm token') ;
              return ;
            }
            setPermState('allow') ;

            console.log('sending new token to server') ;
            $.post('/api/push', {'token': token}, function (d) {
              if (d.error) {
                console.log('server error: ' + d.error) ;
                return ;
              }
        ...
        function getPermission() {
          if (oldPerm == 'default') {
            var oldTs = window.localStorage.getItem('msg_ts') ;
            if (oldTs) {
              var old_d = new Date(parseFloat(oldTs)) ;
              if (((new Date()) - old_d) < 86400000) {
                console.log("Won't request msg permission yet") ;
                return ;
              }
            }
          }
          ...
          fcm.requestPermission().then(function() {
            ...
            setPermState(err.code == 'messaging/permission-default' ?
              'default' : 'block') ;
          });
        }
